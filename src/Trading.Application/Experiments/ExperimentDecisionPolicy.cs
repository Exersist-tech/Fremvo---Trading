...
public sealed class ExperimentDecisionEvidence
{
    internal ExperimentDecisionEvidence(
        ...
        DateTimeOffset asOfUtc, string symbol, CandleInterval interval, DateTimeOffset openTimeUtc, DateTimeOffset closeTimeUtc)
    {
        ...
        OpenTimeUtc = openTimeUtc; CloseTimeUtc = closeTimeUtc;
    }
...
public enum ExperimentProposalAction { Neutral = 0, Open, Reduce, Close }
...
/// never computes quantity, and never emits Add: growing an existing position awaits a weighted ledger.
/// </summary>
public sealed class ExperimentDecisionPolicy
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var proposal = Map(analysis, portfolio);
        var fingerprint = $"{analysis.Outcome}|{analysis.Value?.ToString(System.Globalization.CultureInfo.InvariantCulture)}|{analysis.Reason}|{evidence.ParametersFingerprint}";
    ...
    private static ExperimentProposal Map(ExperimentAnalysisResult analysis, ExperimentWorkerPortfolioSnapshot portfolio)
    {
        ...
        if (analysis.Value > 0m)
            return portfolio.PositionQuantity == 0m
                ? new(ExperimentProposalAction.Open, analysis.Reason)
                : new(ExperimentProposalAction.Neutral, "Existing exposure is never increased by experiment decision policy.");
        return portfolio.PositionQuantity > 0m
